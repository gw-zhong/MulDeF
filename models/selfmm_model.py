...
class SELF_MM(nn.Module):
    def __init__(self, config):
        ...
        self.feature_map = {
            'fusion': to_gpu(torch.zeros(config.train_samples, post_fusion_dim, requires_grad=False),
                             gpu_id=config.gpu_id),
            'text': to_gpu(torch.zeros(config.train_samples, post_text_dim, requires_grad=False), gpu_id=config.gpu_id),
            'audio': to_gpu(torch.zeros(config.train_samples, post_audio_dim, requires_grad=False),
                            gpu_id=config.gpu_id),
            'vision': to_gpu(torch.zeros(config.train_samples, post_video_dim, requires_grad=False),
                             gpu_id=config.gpu_id),
        }
        self.label_map = {
