    ...
    def __init__(self, config):
        ...
        if self.config.misa_use_bert:
            ...
            self.project_t.add_module('project_t',
                                      nn.Linear(in_features=self.bert_text_size, out_features=self.hidden_size))
            self.project_t.add_module('project_t_activation', self.activation)
            self.project_t.add_module('project_t_layer_norm', nn.LayerNorm(self.hidden_size))
        else:
            self.project_t = nn.Sequential()
            self.project_t.add_module('project_t',
                                      nn.Linear(in_features=self.text_size * 4, out_features=self.hidden_size))
            self.project_t.add_module('project_t_activation', self.activation)
            self.project_t.add_module('project_t_layer_norm', nn.LayerNorm(self.hidden_size))

        self.project_v = nn.Sequential()
        self.project_v.add_module('project_v',
                                  nn.Linear(in_features=self.visual_size * 4, out_features=self.hidden_size))
        ...
        self.project_a.add_module('project_a',
                                  nn.Linear(in_features=self.acoustic_size * 4, out_features=self.hidden_size))
        ...
        self.private_t2.add_module('private_t2_1',
        ...
        self.fusion.add_module('fusion_layer_1',
                               nn.Linear(in_features=self.hidden_size * 6, out_features=self.hidden_size * 3))
        ...
        #                        nn.Linear(in_features=self.hidden_size * 3, out_features=self.output_size))
        self.classifier = nn.Linear(in_features=self.hidden_size * 3, out_features=self.output_size)
