...
class MAG(nn.Module):
    def __init__(self, config, args):
        ...
        visual_dim = args.visual_size
        acoustic_dim = args.acoustic_size
        text_dim = args.bert_text_size
        ...
        self.beta_shift = args.magbert_beta_shift
        print(config.hidden_size)
        self.LayerNorm = nn.LayerNorm(config.hidden_size)
        self.dropout = nn.Dropout(args.magbert_dropout)
    ...
    def forward(self, text_embedding, visual, acoustic):
        ...
        hm_norm_ones = to_gpu(torch.ones(hm_norm.shape, requires_grad=True), gpu_id=self.args.gpu_id)
        hm_norm = torch.where(hm_norm == 0, hm_norm_ones, hm_norm)

        thresh_hold = (em_norm / (hm_norm + eps)) * self.beta_shift

        ones = to_gpu(torch.ones(thresh_hold.shape, requires_grad=True), gpu_id=self.args.gpu_id)
...
class MAG_BertForSequenceClassification(BertPreTrainedModel):
    ...
    Returns:
        ...
        return outputs, pooled_output, t_rep
...
class BERT_MAG(nn.Module):
    def __init__(self, args):
        ...
        self.model = MAG_BertForSequenceClassification.from_pretrained(
            self.pretrainedBertPath, args=args, num_labels=args.output_size,
        )

    def forward(self, batch_sample, epoch_info):
        video = batch_sample['visual']
        audio = batch_sample['audio']
        input_ids = batch_sample['bert_sentences']
        input_mask = batch_sample['bert_sentence_att_mask']
        segment_ids = batch_sample['bert_sentence_types']

        output, m_rep, t_rep = self.model(
            input_ids,
            video,
            audio,
